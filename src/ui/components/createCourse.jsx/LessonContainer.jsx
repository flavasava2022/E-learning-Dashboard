...
const LessonContainer = memo(function LessonContainer({
  ...
  onDelete,
  onUpdateLesson,
  // --- New props from the SortableItem wrapper ---
  dragAttributes,
  dragListeners,
}) {
  ...
  // Common styles for the action icons
  const iconButtonSx = {
  ...
  return (
    <>
      ...
      >
        <Stack direction="row" alignItems="center" spacing={{ xs: 1, sm: 1.5 }} sx={{ flexGrow: 1 }}>
          {/* Drag Handle */}
        ...
        <Stack direction="row" sx={{display:{xs:'none',sm:'flex'}}}>
          <Tooltip title="Edit Lesson">
            <IconButton size="small" sx={iconButtonSx} onClick={() => setEditLessonModalOpen(true)}>
      ...
      {/* The modal now gets the onUpdateLesson callback */}
      <EditLessonModal
